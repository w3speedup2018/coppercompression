...
theme.Helpers = (function () {
  ...
  function addToCart(
    ...
    if (isForm) {
      ...
      itemData = theme.Helpers.serialize(itemData);
    }
    ...
    fetch("/cart/add.js", {
      ...
      .then((json) => {
        if (json.status && json.status !== 200) {
          const error = new Error(json.description);
...
theme.Header = (function () {
  ...
  function initAnnouncementSlider() {
    ...
    if (announcementBarSlider !== null) {
      const sliderSpeed = announcementBarSlider.getAttribute(
        "data-announcement-speed"
      );
      new theme.Slideshow(announcementBarSlider, {
        slidesToShow: 1,
        slideInterval: sliderSpeed,
        autoplay: true,
      });
    }
  }
...
class LocalVideo {
  constructor(key, args) {
    this.videoEl = args.video.querySelector("video");
  }
...
theme.Video = (function () {
  ...
  function init(video) {
    ...
    if (video.dataset.type === "local") {
      youtubeLoaded = true;
      onPlayerReady({
        target: {
          getVideoData: () => videos[video.id],
          setPlaybackQuality: () => {},
          getIframe: () => {
            return { id: video.id };
          },
    ...
    if (!youtubeLoaded && video.dataset.type === "youtube") {
  ...
  function privatePlayVideo(id, clicked) {
    ...
    const videoWrapper = videoData.videoWrapper;
  ...
  function onPlayerReady(evt) {
    ...
    const videoData = getVideoOptions(evt);
    const videoTitle = evt.target.getVideoData().title;
    playOnClickCheck();
  ...
  function setAsPlaying(videoData) {
    const videoWrapper = videoData.videoWrapper;
    const pauseButton = videoWrapper.querySelector(selectors.pauseVideoBtn);
    ...
    if (videoData.type === "image_with_play" || videoData.type === "local") {
  ...
  function closeVideo(playerId) {
    const videoData = videos[playerId];
    const videoWrapper = videoData.videoWrapper;
  ...
  function getVideoOptions(evt) {
    const id = evt.target.getIframe().id;
    return videos[id];
  }
  ...
  function toggleExpandVideo(playerId, expand) {
    ...
    const playButton = video.videoWrapper.querySelector(selectors.playVideoBtn);
    let offset = 0;
    let newHeight = 0;
  ...
  function togglePause(playerId) {
    const pauseButton = videos[playerId].videoWrapper.querySelector(
      selectors.pauseVideoBtn
    );
    const paused = pauseButton.classList.contains(classes.userPaused);
    if (paused) {
      pauseButton.classList.remove(classes.userPaused);
  ...
  const handleVideoPlayerKeydown = function (evt) {
    const playerId = document.activeElement.dataset.controls;
  ...
  function sizeBackgroundVideo(videoPlayer) {
    ...
    if (isMobile()) {
      ...
      const videoWrapper = videoPlayer.closest(selectors.videoWrapper);
      ...
      if (videoWidth / videoOptions.ratio < desktopHeight) {
        ...
        const styles = `width: ${playerWidth}px; height: ${desktopHeight}px; left: ${
        ...
        const styles2 = `width: ${videoWidth}px; height: ${desktopHeight}px; top: ${
  ...
  const handleWindowResize = theme.Helpers.debounce(() => {
    if (!youtubeLoaded) {
      return;
    }
    let key;
    ...
    if (isMobile()) {
      ...
      setAutoplaySupport(true);
      for (key in videos) {
        const videosWithImage = videos[key].videoWrapper.querySelectorAll(
  ...
  const handleWindowScroll = theme.Helpers.debounce(() => {
    if (!youtubeLoaded) {
      return;
    }
    ...
    for (const key in videos) {
      if (videos.hasOwnProperty(key)) {
        const videoWrapper = videos[key].videoWrapper;
        const condition =
  ...
  function initEvents() {
    const playVideoBtns = document.querySelectorAll(selectors.playVideoBtn);
    const closeVideoBtns = document.querySelectorAll(selectors.closeVideoBtn);
    const pauseVideoBtns = document.querySelectorAll(selectors.pauseVideoBtn);

    playVideoBtns.forEach((btn) => {
      btn.addEventListener("click", (evt) => {
        const playerId = evt.currentTarget.dataset.controls;
  ...
  function createPlayer(key) {
    const args = Object.assign(videoOptions, videos[key]);

    args.playerVars.controls = args.controls;
    if (videos[key].type !== "local") {
      videoPlayers[key] = new YT.Player(key, args);
    } else {
      videoPlayers[key] = new LocalVideo(key, args);
    }
  }
  ...
  function setButtonLabels(videoWrapper, title) {
    const playButtons = videoWrapper.querySelectorAll(selectors.playVideoBtn);
    const closeButton = videoWrapper.querySelector(selectors.closeVideoBtn);
    const pauseButton = videoWrapper.querySelector(selectors.pauseVideoBtn);
    const closeButtonText = closeButton.querySelector(selectors.fallbackText);

    const pauseButtonStop = pauseButton.querySelector(selectors.pauseVideoStop);
    const pauseButtonStopText = pauseButtonStop.querySelector(
      selectors.fallbackText
    );

    const pauseButtonResume = pauseButton.querySelector(
      selectors.pauseVideoResume
    );
    const pauseButtonResumeText = pauseButtonResume.querySelector(
      selectors.fallbackText
    );
    ...
    playButtons.forEach((playButton) => {
      const playButtonText = playButton.querySelector(selectors.fallbackText);
...
theme.Hero = (function () {
  ...
  function hero(el, sectionId) {
    ...
    var parentSection =
      ...
      document.querySelector("#hero-section-" + sectionId);
...
theme.SearchResultsTemplate = (function () {
  function renderResults(products, articles, isLoading, searchQuery) {
    return [
      '<div class="predictive-search">',
      renderHeader(products, isLoading),
      renderProducts(products, articles, searchQuery),
      "</div>",
    ].join("");
  }
  ...
  function renderProducts(products, articles, searchQuery) {
    var resultsCount = products.length + articles.length;
  ...
  function renderArticle(article, index, resultsCount) {
    return [
      '<li id="search-result-' +
        index +
        '" class="predictive-search-item" role="option" data-search-result>',
  ...
  function normalizeProduct(product) {
    ...
    return {
      ...
      title: product.title.replace(/Copper Compression/i, ""),
  ...
  function normalizeArticle(article) {
    const featuredImage = article.featured_image.url
      ? {
  ...
  return function (data) {
    ...
    let articles = data.articles ? data.articles.slice(0, 2) : [];
    ...
    return renderResults(products, articles, isLoading, searchQuery);
  };
})();
...
theme.SearchDrawer = (function () {
  ...
  function setHeight() {
    ...
    var iconsHeight = document.querySelector(
      ".site-header__icons"
    ).offsetHeight;

    searchDrawer.style.height = iconsHeight + "px";
    searchDrawer.style.top = headerHeight - iconsHeight + "px";
  }
...
theme.customerTemplates = (function () {
  ...
  /**
   *  Check URL for checkout parameter on login page
   */
  function checkLoginParameters() {
    if (!window.location.href.includes("/login?checkout_url")) {
      return;
    }
    const registerLink = document.getElementById("customer_register_link");
    registerLink.setAttribute("href", "/account/register?return_to=checkout");
  }

  /**
   *  Check URL for checkout parameter on register page
   */
  function checkRegisterParameters() {
    if (!window.location.href.includes("/register?return_to=checkout")) {
      return;
    }
    const redirectInput = document.querySelector("[name='return_to']");
    redirectInput.remove();
  }

  return {
    init: function () {
      initEventListeners.bind(this)();
      checkUrlHash.bind(this)();
      resetPasswordSuccess.bind(this)();
      customerAddressForm.bind(this)();
      checkLoginParameters.bind(this)();
      checkRegisterParameters.bind(this)();
    },
  };
})();
...
theme.Cart = (function () {
  var selectors = {
    ...
    cartPreheader: "[data-cart-preheader]",
    cartPreheaderSubtotal: "[data-cart-preheader-subtotal]",
    cartPreheaderCount: "[data-cart-preheader-item-count]",
    ...
    decrementQuantityButton: "[data-quantity-decrement]",
    incrementQuantityButton: "[data-quantity-increment]",
  ...
  function Cart(container) {
    ...
    this._handleQtyAdjust = this._handleQtyAdjust.bind(this);
    ...
    this.container.addEventListener("click", this._handleQtyAdjust);
  ...
  Cart.prototype = Object.assign({}, Cart.prototype, {
    ...
    _handleQtyAdjust: function (evt) {
      if (
        !evt.target.hasAttribute("data-quantity-decrement") &&
        !evt.target.hasAttribute("data-quantity-increment")
      )
        return;
      evt.preventDefault();
      ...
      var itemQtyInput = itemElement.querySelector("[data-quantity-item]");

      var value = parseInt(itemQtyInput.value);
      ...
      if (input.hasAttribute("data-quantity-decrement")) {
        itemQtyInput.value = --value;
      } else {
        itemQtyInput.value = ++value;
      }

      this._handleInputQty({ target: itemQtyInput });
    },
      ...
      fetch("/cart/change.js", request)
        ...
        .then(
          function (state) {
            ...
            this._setFreeShippingBar(state.total_price);
    ...
    _createCart: function (state) {
      ...
      this.container.querySelector(selectors.cartSubtotal).innerHTML =
        theme.Currency.formatMoney(
          state.total_price,
          theme.moneyFormatWithZeroes
        );

      this.container.querySelector(selectors.cartPreheaderSubtotal).innerHTML =
        theme.Currency.formatMoney(
          state.total_price,
          theme.moneyFormatWithZeroes
        );
    ...
    _createLineItemList: function (state) {
      return state.items.map(
        function (item, index) {
          ...
          cartItemTitle.textContent = item.product_title.replace(
            /copper compression/i,
            ""
          );
    ...
    _setQuantityInputs: function (itemNode, item, index) {
      ...
      desktopInput.setAttribute(attributes.quantityItem, index + 1);
      desktopInput.value = item.quantity;
    ...
    _setCartCountBubble: function (quantity) {
      ...
      this.cartPreheaderCount =
        this.cartPreheaderCount ||
        document.querySelector(selectors.cartPreheaderCount);
      ...
      if (quantity === 1) {
        this.cartPreheaderCount.textContent = theme.strings.oneCartCount;
      } else {
        this.cartPreheaderCount.textContent =
          theme.strings.otherCartCount.replace("[count]", quantity);
      }
    },

    _setFreeShippingBar: function (total_price) {
      const freeShippingBar = document.querySelector(
        ".announcement-bar__free-shipping__text"
      );
      if (!freeShippingBar) return;
      if (total_price < 5000) {
        freeShippingBar.textContent =
          theme.strings.amount_to_qualify_for_free_shipping.replace(
            "[amount]",
            theme.Currency.formatMoney(5000 - total_price, theme.moneyFormat)
          );
      } else {
        freeShippingBar.textContent = theme.strings.qualifies_for_free_shipping;
      }
    },
    ...
    _emptyCart: function () {
      ...
      this.cartPreheader =
        this.cartPreheader ||
        this.container.querySelector(selectors.cartPreheader);

      this.emptyPageContent.classList.remove(classes.hide);
      this.cartWrapper.classList.add(classes.hide);
      this.cartPreheader.classList.add(classes.hide);
    },
  });
...
theme.Product = (function () {
  function Product(container) {
    ...
    this.settings = {
      ...
      mediaQueryMediumUp: "screen and (min-width: 993px)",
      mediaQuerySmall: "screen and (max-width: 992px)",
      bpSmall: false,
      enableHistoryState:
    ...
    this.selectors = {
      ...
      sizeGuideButton: ".size-guide-trigger",
    ...
    this.quantityDecrement = container.querySelector(
      this.selectors.decrementQuantityButton
    );
    this.quantityIncrement = container.querySelector(
      this.selectors.incrementQuantityButton
    );
    ...
    this.sizeTrigger = container.querySelector(this.selectors.sizeGuideButton);
    this.sizeModal = container.querySelector(this.selectors.sizeGuideModal);
  ...
  Product.prototype = Object.assign({}, Product.prototype, {
    ...
    _initStoreAvailability() {
      this.storeAvailability = new theme.StoreAvailability(
        this.storeAvailabilityContainer
      );

      const storeAvailabilityModalOpenedCallback = function (event) {
        if (
    ...
    _initMobileBreakpoint() {
      if (this.mqlSmall.matches) {
        ...
        if (this.settings.zoomEnabled) {
          this.imageZoomWrapper.forEach((element, index) => {
    ...
    _initMediaSwitch() {
      ...
      const productThumbImages = document.querySelectorAll(
      ...
      productThumbImages.forEach((el) => {
        el.addEventListener("click", (evt) => {
          evt.preventDefault();
          const mediaId = el.getAttribute("data-thumbnail-id");
          self.jumpToMediaId = mediaId;
    ...
    _initQuantity() {
      ...
      this.quantityDecrement.addEventListener("click", (evt) => {
        evt.preventDefault();
        let quantity = parseInt(self.quantityInput.value);
        if (isNaN(quantity) || quantity < 2) {
          quantity = 2;
        }
        quantity--;
        self.quantityInput.value = quantity;
      });

      this.quantityIncrement.addEventListener("click", (evt) => {
        evt.preventDefault();
        let quantity = parseInt(self.quantityInput.value);
        if (isNaN(quantity)) {
          return;
    ...
    _initSizeGuide() {
      if (!document.querySelector(this.selectors.sizeGuideButton)) {
        return;
      }

      const self = this;

      this.sizeTrigger.addEventListener("click", (evt) => {
        window.scrollTo(0, 0);
        document.body.classList.add("stop-scroll");
        self.sizeModal.classList.add("size-guide-modal--show");
        evt.preventDefault();
      });

      this.sizeModal.addEventListener("click", (evt) => {
        document.body.classList.remove("stop-scroll");
        self.sizeModal.classList.remove("size-guide-modal--show");
        evt.preventDefault();
      });
    },

    _initAddToCart() {
      this.productForm.addEventListener("submit", (evt) => {
        ...
        const isInvalidQuantity =
          Boolean(this.quantityInput) && this.quantityInput.value <= 0;
        ...
        if (!isInvalidQuantity && this.ajaxEnabled) {
          ...
          // request/cart is loading and handle loading state
          this._handleButtonLoadingState(true);
          theme.Helpers.addToCart(
            this.productForm,
            true,
            () => {
    ...
    _initProductVideo() {
      ...
      const productMediaTypeVideo = this.container.querySelectorAll(
        this.selectors.productMediaTypeVideo
      );
      productMediaTypeVideo.forEach((el) => {
    ...
    _handleButtonLoadingState(isLoading) {
      if (isLoading) {
    ...
    _showErrorMessage(errorMessage) {
      const errorMessageContainer = this.container.querySelector(
    ...
    _getVariantOptionList(variantOptions) {
      let variantOptionListHTML = "";
    ...
    _getPropertyList(properties) {
      ...
      propertiesArray.forEach((property) => {
        ...
        propertyListHTML =
          `${propertyListHTML}<li class="product-details__item product-details__item--property">` +
          `<span class="product-details__property-label">${property[0]}: </span>${property[1]}`;
        ": " + "</li>";
      });
    ...
    _getSellingPlanHTML(selling_plan_name) {
      const sellingPlanHTML = `<li class="product-details__item product-details__item--property">${selling_plan_name}</li>`;
    ...
    _hideCartPopup(event) {
      const setFocus = event.detail === 0;
      theme.Helpers.prepareTransition(this.cartPopupWrapper);
      this.cartPopupWrapper.classList.add(this.classes.cartPopupWrapperHidden);

      const cartPopupImage = document.querySelector(
    ...
    _setActiveThumbnail(mediaId) {
      ...
      if (typeof mediaId === "undefined") {
        const productMediaWrapper = this.container.querySelector(
      ...
      const thumbnailWrappers = this.container.querySelectorAll(
        `${this.selectors.productThumbListItem}:not(.slick-cloned)`
      );

      let activeThumbnail;
      thumbnailWrappers.forEach((el) => {
    ...
    _adjustThumbnailSlider(activeThumbnail) {
      const sliderItem = activeThumbnail.closest("[data-slider-item]");
      if (!sliderItem) {
        return;
      }

      const sliderTrack = sliderItem.closest("[data-slider-track]");

      sliderTrack.scrollTo({
        left:
          sliderItem.offsetLeft -
          sliderTrack.offsetWidth / 2 +
          sliderItem.offsetWidth / 2,
        behavior: "smooth",
      });

      const slideGroupLeaderIndex =
        Math.floor(
    ...
    _switchMedia(mediaId) {
      ...
      const newMedia = this.container.querySelector(
        `${this.selectors.productMediaWrapper}[data-media-id='${mediaId}']`
      );

      const mediaGroup = this.container.querySelector(
    ...
    _handleMediaFocus(evt) {
      if (evt.keyCode !== slate.utils.keyboardKeys.ENTER) {
        return;
      }

      const mediaId = evt.currentTarget.getAttribute("data-thumbnail-id");
        ...
        (entries) => {
          entries.map((e) => {
            if (e.isIntersecting) {
              ...
              if (this.jumpToMediaId === mediaId) {
                this.jumpToMediaId = null;
                return;
              } else if (this.jumpToMediaId) {
                return;
              }
              this._setActiveThumbnail(e.target.dataset.mediaId);
            }
          });
        },
        {
          threshold: 0.75,
          root: this.container.querySelector(this.selectors.productMediaGroup),
        }
      );
      this.container
        .querySelectorAll(this.selectors.productMediaWrapper)
    ...
    _destroyThumbnailSlider() {
      const sliderButtons = this.container.querySelectorAll(
        "[data-slider-button]"
      );
      const sliderTrack = this.container.querySelector("[data-slider-track]");
      const sliderItems = sliderTrack.querySelectorAll("[data-slider-item");
      this.settings.sliderActive = false;

      if (sliderTrack) {
        sliderTrack.removeAttribute("style");
        sliderItems.forEach((sliderItem) => {
          const sliderItemLink = sliderItem.querySelector(
    ...
    _liveRegionText(variant) {
      ...
      // Update availability
      const availability = this.productState.soldOut
      ...
      if (this.productState.showUnitPrice) {
        ...
        unitPrice = `${theme.Currency.formatMoney(
          variant.unit_price,
          theme.moneyFormat
        )} ${theme.strings.unitPriceSeparator} ${this._getBaseUnit(variant)}`;
      }
    ...
    _setProductState(evt) {
      ...
      this.productState.showUnitPrice = Boolean(variant.unit_price);
    },

    _updateAvailability(evt) {
    ...
    _updateStoreAvailabilityContent(evt) {
    ...
    _updateMedia(evt) {
      ...
      const sectionMediaId = `${this.settings.sectionId}-${mediaId}`;
    ...
    _updatePriceComponentStyles(evt) {
      const variant = evt.detail.variant;
    ...
    _updatePrice(evt) {
      ...
      const regularPrices = this.priceContainer.querySelectorAll(
        this.selectors.regularPrice
      );
      const salePrice = this.priceContainer.querySelector(
        this.selectors.salePrice
      );
      const unitPrice = this.priceContainer.querySelector(
        this.selectors.unitPrice
      );

      const formatRegularPrice = function (regularPriceElement, price) {
    ...
    _updateSKU(evt) {
      ...
      // Update the sku
      const sku = document.querySelector(this.selectors.SKU);
      if (!sku) {
        return;
      }
    ...
    _initPopupLink() {
      // TODO determine if klaviyo user already exists
      // As is this keeps the last chance signup link hidden to all users

      const popupLink = document.getElementById("klaviyo-last-chance");
      const existingKlaviyoUser = true;
      if (!existingKlaviyoUser) {
        popupLink.classList.remove("no-show");
      }
    },
    onUnload() {
      this.container.removeEventListener(
